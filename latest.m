...
disp("      did ROI");
    ...
    for ifp=1:length(kkx)
        plot(kkx(ifp),kky(ifp),'r-o');
        plot([ROI{ifp}(1),ROI{ifp}(1)+sizex],[ROI{ifp}(2),ROI{ifp}(2)+sizey], "LineWidth", 3);
    end
...
for nj=i:i+9
     figure;
        ...
        hold on;
        for b=1:sizeROI(1,2)
            points=detectHarrisFeatures(edgeimglist{picnum},'MinQuality',0.15,'FilterSize', 47,'ROI',ROI{b});
